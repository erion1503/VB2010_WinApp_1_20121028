...
Public Class ReverseGrid
    ...
    Dim m_Cells(XCount - 1, YCount - 1) As Cell '全セルをあらわす配列
    '■コンストラクタ
    ...
    ''' <param name="Status">ひっくり返す原因となった石の色を判定します。</param>
    ''' <param name="Direction">ひっくり返す方向を指定します。</param>
    ''' <param name="X">対象のセルの0から始まるX位置をしてします。Xの最大値はXCount - 1です。</param>
    ''' <param name="Y">対象のセルの0から始まるY位置をしてします。Yの最大値はYCount - 1です。</param>
    Public Sub Reverse(ByVal Status As CellStatus, ByVal Direction As ScanDirection, ByVal X As Integer, ByVal Y As Integer)

        Dim AnotherStatus As CellStatus
        Dim i As Integer
        Dim XDirection As Integer '左のとき、-1,右のとき、1
        Dim YDirection As Integer '上のとき、-1,右のとき、1
    ...
    ''音声ファイルを読み込む
    'Private WhiteVoice As UnmanagedMemoryStream = My.Resources.バルサミコ酢
    ''///▲TASK＿POINT
    ''WAVEファイルを再生する
    ...
    '    '読み込む
    '    player = New System.Media.SoundPlayer(WhiteVoice)
    '    '非同期再生する
    '    player.Play()
    ...
    ''' <param name="X">対象セルの0から始まるX位置を指定します。Xの最大値はXCount-1です。</param>
    ''' <param name="Y">対象セルの0から始まるY位置を指定します。Yの最大値はYCount-1です。</param>
    ''' <returns>石を置いた場合Trueを返します。置けなかった場合はFalseを返します。</returns>
    ''' <remarks>このメソッドを呼び出すと周辺の石をひっくり返します。</remarks>
    ...
    '■Count
    ''' <Summary>状態を指定してセルを取得します。</Summary>
    ...
    '■PuttableCount
    ''' <summary>置くことができる石の数を調べます。</summary>
    ''' <param name="Status">調べる石の色を指定します。</param>
    ''' <returns>Statusで指定された色を置くことできる数を返します。</returns>
